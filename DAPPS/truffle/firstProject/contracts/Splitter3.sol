...
/**
This import was giving me an error - 'SplitFundLib.sol' not found: 
File import callback not supported. 
hence commented out
 */
//import "./SplitFundLib.sol";
...
contract Splitter3{
    ...
    SplitFundLib.Fund addressBalance;
    ...
    event LogFundSplit(address from, uint amount, address receiver1, address receiver2);
    event LogBalance(address addr, uint balanceAmount);
    ...
    function deposit (uint amount) public payable returns(bool){
        ...
        if(msg.sender == Alice) {
            ...
            SplitFundLib.splitFund(addressBalance, Bob, Carol, msg.value);
        }
        else {
