...
/**
*@title Pausable
*@dev This contract contains feature to pause the contract functionalities.
...
/**
@dev this function will update the toll rate for vehicle type. The new rate is calculated based on business logic and different index. Couple of indexes used are Customer Price Index (CPI) or Average Weekly Earnings (EWE) or combination of both. Only current contract owner can invoke this function.
@param vehicleType
@return newRate
*/
...
/**
@dev this function will create the new road operator. Only current contract owner can invoke this function.
@param roadOperatorAddress
@return roadOperator
*/
...
@title RoadOperator
@author Nitish Bhushan
@dev Below are its main responsibilities
- Instantiate and track toll booth operators. Each toll booth is identified by its unique Id and this information is publicly available
...
- process tag id, before entering to toll road, capture their toll journey details
- process linked account, fetch its balance and block the approx toll fee amount
- At the exit, calculate final toll fee, do accounting, returns balance, and update the system.

*/
contract TollBoothOperator is Pausable {

/**
@dev This event is emitted when tag id of the approaching vehicle is validated. System would get the linked vehicle registartion from the subsystem for this tagid and match it against the vehicle actual registartion number. If its match then true will return otherwise false would return. Also the details would run through anti theft subsystem. 
@param tollBoothOperatorAddress
@param tagId
@param vehicleType
@param dateTime
@param status
 */
event LogVehicleTagIdInspected(address tollBoothOperatorAddress, bytes32 tagId, bytes32 vehicleType, uint dateTime, bool status);


/**
@dev This event is emitted when vehicle enters the toll road and their information is captured. 
@param tollBoothOperatorAddress
@param tagId 
@param vehicleType
@param entryTollBoothId
...
@param dateTime
 */
event LogVehicleEntered(address tollBoothOperatorAddress, bytes32 tagId, bytes32 vehicleType, bytes32 vehicleRegistrationNo,  bytes32 entryTollBoothId, uint dateTime);


/**
@dev This event is emitted when account balance is checked and apprx toll fee amount is blocked.
@param tollBoothOperatorAddress
@param tagId 
@param vehicleType
@param entryTollBoothId
...
@param dateTime
 */
event LogTollFeeAmountBlocked(address tollBoothOperatorAddress, bytes32 tagId, bytes32 vehicleType, bytes32 entryTollBoothId uint dateTime, address ethereumAddress, uint blockedAmount);
 ...
 */
event LogVehicleExitedAndActualTollFeeProcessed(address tollBoothOperatorAddress, bytes32 tagId, bytes32 vehicleType, bytes32 entryTollBoothId, bytes32 exitTollBoothId, uint dateTime, address ethereumAddress, uint blockedAmount, uint ActualTollFee, uint refundAmount);
...
*/
function enquireAccBalanceAndBlockAmount(bytes32 tagId, bytes32 vehicleType, bytes32 entryTollBoothId uint dateTime) public onlyWhenNotPaused view returns(uint blockedAmount);
...
@return
*/
function processExitingVehicle(bytes32 tagId, bytes32 vehicleType, bytes32 vehicleRegistrationNo, bytes32 entryTollBoothId, bytes32 exitTollBoothId, uint dateTime) public onlyWhenNotPaused view returns(bool status);
